...
//  2.  mode可选"r"(read),"w"(write),"a"(append)...后可加"b"(binary),"+"
//      (update)或"b+"
//----------------------------------读写流----------------------------------
...
//  5.  scanf：%[*][width][length]specifier. *表读指定类型但不保存，width表读入
//      最大字符数；%[ABC]仅读ABC三种字符，%[A-Z]只读大写字母，%[^ABC]表过滤ABC
//  6.  printf：%[flags][width][.precision][length]specifier.
//          [flags]：    -左对齐；+数字符号强制显示；0数前补0至列宽；(空格)正数前加空
//                      格负数前加负号；#类型o/x/X前加0/0x/0X，类型e/E/f/g/G强制
//                      输出小数点，类型g/G保留尾部0
//          specifier：  d有符号十进制整；u无符号10进制整；o无符号8进制整；x/X无符号
